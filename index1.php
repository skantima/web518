...
if(!empty($_POST["submit_email"])) {
	$result = mysqli_query($dbc,"SELECT * FROM users WHERE email_id='" . $_POST["email"] . "'");
	...
	if($count>0) {
		...
		if($mail_status == 1) {
			$result = mysqli_query($dbc,"INSERT INTO otp_expiry(otp,is_expired,create_at) VALUES ('" . $otp . "', 0, '" . date("Y-m-d H:i:s"). "')");
...
if(!empty($_POST["submit_otp"])) {
	...
	$count  = mysqli_num_rows($result);
	if(!empty($count)) {
		$result = mysqli_query($dbc,"UPDATE otp_expiry SET is_expired = 1 WHERE otp = '" . $_POST["otp"] . "'");
		$success = 2;	
	} else {
		$success =1;
